...
//большой фон слотов
    ...
    format(string, sizeof string, "%s %.1f / %d.0 kg", 
        InventoryTextTD[0][PlayerInv[playerid][iLanguage]], 
        KgBackpack, 
        backpack, 
        InventoryTextTD[10][PlayerInv[playerid][iLanguage]]);
...
// === индикаторы здоровья, сытости
    ...
    for(new x; x < 4; x++) {
        ...
        if(x < 3) {
            ...
            switch(x) {
                case 0: {
                    PlayerInv[playerid][iIndicators_Bar][x] = CreatePlayerProgressBar(playerid, 602.00, PosY + 4.0, 34.00, 1.69, 0xFF0000FF); 
                }
                case 1: {
                    PlayerInv[playerid][iIndicators_Bar][x] = CreatePlayerProgressBar(playerid, 602.00, PosY + 4.0, 34.00, 1.69, 0xFFFF00FF);
                }
                case 2: {
                    PlayerInv[playerid][iIndicators_Bar][x] = CreatePlayerProgressBar(playerid, 602.00, PosY + 4.0, 34.00, 1.69, 0x0075FFFF);
                }
            }
